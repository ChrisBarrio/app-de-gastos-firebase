...
const Alerta = ({ tipo, mensaje, estadoAlerta, cambiarEstadoAlerta }) => {
  useEffect(() => {
    ...
    if (estadoAlerta === true) {
      //luego de 4seg cambio el estado para que vuelva a funcionar el boton.
      tiempo = setTimeout(() => {
        cambiarEstadoAlerta(false);
      }, 4000);
    }
    //pasamos una funcion de limpieza. Esta funcion se ejecuta si el componente se desmonta.
    //lo que hacemos es limpiar el tiempo para que no intente cambiar estado si el componente no esta en pantalla.
